...
const Members = ({ members, isLoading }: { members: HubOSAutID<AutIDProperties>[], isLoading: boolean }) => {
  ...
  const { tabs, firstTabWithMembers } = useMemo(() => {
    ...
    const firstTab = Object.keys(groupedMembers).findIndex(
      (role) => groupedMembers[role].length > 0
    );

    return {
      tabs: generateMemberTabs(groupedMembers),
      firstTabWithMembers: firstTab
    };
  }, [hubData, members]);

  useEffect(() => {
    if (firstTabWithMembers >= 0) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setSelectedTabIndex(firstTabWithMembers);
    }
  }, [firstTabWithMembers]);
